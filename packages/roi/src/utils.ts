...
/**
 * Format EUR currency with Dutch locale support
 * Dutch format: €12.500,00 (dot for thousands, comma for decimals)
 * English format: €12,500.00 (comma for thousands, dot for decimals)
 */
export function formatCurrency(value: number, locale: 'nl' | 'en' = 'nl'): string {
  const formatted = new Intl.NumberFormat(locale === 'nl' ? 'nl-NL' : 'en-US', {
    style: 'currency',
    currency: 'EUR',
 ...
 */
export function formatNumber(value: number, locale: 'nl' | 'en' = 'nl'): string {
  return new Intl.NumberFormat(locale === 'nl' ? 'nl-NL' : 'en-US').format(value)
}
